...
class SfMDataset(object):
    ...
    def parser(self, serialized):
        ...
        if self.flip_pair:
            ...
            is_flip = tf.less_equal(tf.random_uniform([]), 0.5) #<= [0,1)
    ...
    def _decode_depth(self, filename, shape):
        ...
        depth = tf.scalar_mul(self.depth_factor, tf.cast(depth, tf.float32))
        is_zero = tf.equal(depth, tf.constant(0, dtype=tf.float32))
        valid_mask = tf.cast(tf.logical_not(is_zero), tf.float32) # 0->1
        far_depth = tf.scalar_mul(self.far_depth_val, tf.ones_like(depth, dtype=tf.float32))#1000
        depth = tf.where(is_zero, far_depth, depth) #max 1000 else normal
        return depth, valid_mask
