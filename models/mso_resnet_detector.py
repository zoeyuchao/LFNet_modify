...
def get_model(inputs, is_training, 
    ...
    with tf.variable_scope(name, reuse=reuse) as net_sc:
        curr_in = tf.identity(inputs) #[6,256,256,1]

        # init-conv channel=16 kernel=5    this function is in the tf_layer_utils.py
        ...
        if num_scales == 1:
            ...
            scale_log_factors = np.linspace(np.log(max_scale), np.log(min_scale), num_scales)#5
        ...
        for i, s in enumerate(scale_factors):
            ...
            score_maps = conv2d_fixed_padding(rs_feat_maps, 1, 
                        ...
                        use_xavier=use_xavier, use_bias=use_bias)
            #print('score_maps = {}'.format(score_maps))
...
class Model(object):
    ...
    def build_model(self, photos, reuse=False, name='ConvOnlyResNet'):
        ...
        num_scales = self.config.net_num_scales#5
