...
def get_model(inputs, is_training, 
    ...
    #config.desc_net_depth == num_conv_layers = 3
    #config.desc_net_channel == init_num_channels= 64
    channels_list = [init_num_channels * 2**i for i in range(num_conv_layers)]
    print('channels_list = {}'.format(channels_list))
    print('===== {} (reuse={}) ====='.format(name, reuse))

    with tf.variable_scope(name, reuse=reuse) as net_sc:
        curr_in = inputs#[B*K,32,32,1]
        ...
        #print('FLATTEN LAYER {}'.format(curr_in.shape))
        curr_in = fully_connected(curr_in, 512, scope='fc1', 
                                  use_xavier=use_xavier, use_bias=use_bias)
        print('curr_in = {}'.format(curr_in))
        curr_in_zoe = batch_norm_act(curr_in, activation_fn, 
                                 perform_bn=perform_bn,
                                 is_training=is_training, 
                                 bn_decay=bn_decay,
                                 bn_affine=bn_affine,
        ...
        #out_dim=256
        print('curr_in = {}'.format(curr_in_zoe))
        raw_feats = fully_connected(curr_in_zoe, out_dim, scope='fc2',
                                use_xavier=use_xavier, use_bias=use_bias)
        if feat_norm == 'l2norm':#go
...
class Model(object):
    def __init__(self, config, is_training):
        ...
        #config.desc_activ_fn=relu
        #config.desc_leaky_alpha=0.2
